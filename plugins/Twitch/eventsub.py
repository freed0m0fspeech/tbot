...
from .helpers import TWITCH_API_BASE_URL, remove_none_values
from .types import *
import requests
...
class EventSub:
    ...
    callbacks = {}
    active_webhooks = {}
    hook_thread: Union['threading.Thread', None] = None
    hook_loop: Union['asyncio.AbstractEventLoop', None] = None
    hook_runner: Union['web.AppRunner', None] = None
    logger: Logger = None
    ...
    def run_hook(self, runner: 'web.AppRunner'):
        ...
        site = web.TCPSite(runner, str(self._host), self._port, ssl_context=self.ssl_context)
    ...
    def start(self):
        ...
        self.__hook_thread = threading.Thread(target=self.run_hook, args=(self.build_runner(),))
        self.__running = True
    ...
    def api_post_request(self, url: str, data: Union[dict, None] = None):
    ...
    def api_delete_request(self, url: str):
        headers = self.build_request_header()
        return requests.delete(url, headers=headers)

    def add_callback(self, c_id: str, callback):
        self.callbacks[c_id] = {'id': c_id, 'callback': callback, 'active': False}

    def activate_callback(self, c_id: str):
        self.callbacks[c_id]['active'] = True

    def subscribe(self, sub_type: str, sub_version: str, condition: dict, callback) -> str:
        """
        **Subscribe to Twitch Topic**
        ...
        `Channel Update` **channel.update** v.1
        A broadcaster updates their channel properties e.g., category, title,mature flag, broadcast, or language.\n
        `Channel Follow`	**channel.follow**	v.1
        A specified channel receives a follow.\n
        `Channel Subscribe`	**channel.subscribe**	v.1
        A notification when a specified channel receives a subscriber. This does not include resubscribes.\n
        `Channel Subscription End`	**channel.subscription.end**	v.1
        A notification when a subscription to the specified channel ends.\n
        `Channel Subscription Gift`	**channel.subscription.gift**	v.1
        A notification when a viewer gives a gift subscription to one or more users in the specified channel.\n
        `Channel Subscription Message`	**channel.subscription.message**	v.1
        A notification when a user sends a resubscription chat message in a specific channel.\n
        `Channel Cheer`	**channel.cheer**	v.1
        A user cheers on the specified channel.\n
        `Channel Raid`	**channel.raid**	v.1
        A broadcaster raids another broadcaster’s channel.\n
        `Channel Ban`	**channel.ban**	v.1
        A viewer is banned from the specified channel.\n
        `Channel Unban`	**channel.unban**	v.1
        A viewer is unbanned from the specified channel.\n
        `Channel Moderator Add`	**channel.moderator.add**	v.1
        Moderator privileges were added to a user on a specified channel.\n
        `Channel Moderator Remove`	**channel.moderator.remove**	v.1
        Moderator privileges were removed from a user on a specified channel.\n
        `Channel Points Custom Reward Add`	**channel.channel_points_custom_reward.add**	v.1
        A custom channel points reward has been created for the specified channel.\n
        `Channel Points Custom Reward Update`	**channel.channel_points_custom_reward.update**	v.1
        A custom channel points reward has been updated for the specified channel.\n
        `Channel Points Custom Reward Remove`	**channel.channel_points_custom_reward.remove**	v.1
        A custom channel points reward has been removed from the specified channel.\n
        `Channel Points Custom Reward Redemption Add`	**channel.channel_points_custom_reward_redemption.add**	v.1
        A viewer has redeemed a custom channel points reward on the specified channel.\n
        `Channel Points Custom Reward Redemption Update`	**channel.channel_points_custom_reward_redemption.update**	v.1
        A redemption of a channel points custom reward has been updated for the specified channel.\n
        `Channel Poll Begin`	**channel.poll.begin**	v.1
        A poll started on a specified channel.\n
        `Channel Poll Progress`	**channel.poll.progress**	v.1
        Users respond to a poll on a specified channel.\n
        `Channel Poll End`	**channel.poll.end**	v.1
        A poll ended on a specified channel.\n
        `Channel Prediction Begin`	**channel.prediction.begin**	v.1
        A Prediction started on a specified channel.\n
        `Channel Prediction Progress`	**channel.prediction.progress**	v.1
        Users participated in a Prediction on a specified channel.\n
        `Channel Prediction Lock`	**channel.prediction.lock**	v.1
        A Prediction was locked on a specified channel.\n
        `Channel Prediction End`	**channel.prediction.end**	v.1
        A Prediction ended on a specified channel.\n
        `Drop Entitlement Grant`	**drop.entitlement.grant**	v.1
        An entitlement for a Drop is granted to a user.\n
        `Extension Bits Transaction Create`	**extension.bits_transaction.create**	v.1
        A Bits transaction occurred for a specified Twitch Extension.\n
        `Goal Begin`	**channel.goal.begin**	v.1
        Get notified when a broadcaster begins a goal.\n
        `Goal Progress`	**channel.goal.progress**	v.1
        Get notified when progress (either positive or negative) is made towards a broadcaster’s goal.\n
        `Goal End`	**channel.goal.end**	v.1
        Get notified when a broadcaster ends a goal.\n
        `Hype Train Begin`	**channel.hype_train.begin**	v.1
        A Hype Train begins on the specified channel.\n
        `Hype Train Progress`	**channel.hype_train.progress**	v.1
        A Hype Train makes progress on the specified channel.\n
        `Hype Train End`	**channel.hype_train.end**	v.1
        A Hype Train ends on the specified channel.\n
        `Stream Online`	**stream.online**	v.1
        The specified broadcaster starts a stream.\n
        `Stream Offline`	**stream.offline**	v.1
        The specified broadcaster stops a stream.\n
        `User Authorization Grant`	**user.authorization.grant**	v.1
        A user’s authorization has been granted to your client id.\n
        `User Authorization Revoke`	**user.authorization.revoke**	v.1
        A user’s authorization has been revoked for your client id.\n
        `User Update`	**user.update**	v.1
        ...
        result = self.api_post_request(TWITCH_API_BASE_URL + 'eventsub/subscriptions', data=data).json()
        ...
        self.add_callback(sub_id, callback)
        if self.wait_for_subscription_confirm:
            ...
            while timeout >= datetime.datetime.utcnow():
                if self.callbacks[sub_id]['active']:
                    return sub_id
    ...
    async def __handle_default(self, request: 'web.Request'):
        return web.Response(text="Twitch EventSub")
    ...
    async def __handle_challenge(self, request: 'web.Request', data: dict):
        ...
        self.activate_callback(data.get('subscription').get('id'))
    ...
    def unsubscribe_all(self):
        ...
        # get all ids
        while repeat:
            ret = self.twitch.get_eventsub_subscriptions(after=cursor)
            for d in ret.get('data', []):
                ids.append(d.get('id'))
            cursor = ret.get('pagination', {}).get('cursor')
            repeat = cursor is not None
        for _id in ids:
            succ = self.twitch.delete_eventsub_subscription(_id)
            if not succ:
    ...
    def unsubscribe_all_known(self):
        ...
        for key, value in self.callbacks.items():
            ...
            succ = self.twitch.delete_eventsub_subscription(key)
            if not succ:
    ...
    def unsubscribe_topic(self, topic_id: str) -> bool:
        ...
        result = self.twitch.delete_eventsub_subscription(topic_id)
        if result:
            self.callbacks.pop(topic_id, None)
        return result
    ...
    def listen_channel_update(self, broadcaster_user_id: str, callback: CALLBACK_TYPE) -> str:
        ...
        :rtype: str
        """
        return self.subscribe('channel.update', '1', {'broadcaster_user_id': broadcaster_user_id}, callback)
    ...
    def listen_channel_follow(self, broadcaster_user_id: str, callback: CALLBACK_TYPE) -> str:
        ...
        :rtype: str
        """
        return self.subscribe('channel.follow', '1', {'broadcaster_user_id': broadcaster_user_id}, callback)
    ...
    def listen_channel_subscribe(self, broadcaster_user_id: str, callback: CALLBACK_TYPE) -> str:
        ...
        :rtype: str
        """
        return self.subscribe('channel.subscribe', '1', {'broadcaster_user_id': broadcaster_user_id}, callback)
    ...
    def listen_channel_subscription_end(self, broadcaster_user_id: str, callback: CALLBACK_TYPE) -> str:
        ...
        :rtype: str
        """
        return self.subscribe('channel.subscription.end', '1', {'broadcaster_user_id': broadcaster_user_id}, callback)
    ...
    def listen_channel_subscription_gift(self, broadcaster_user_id: str, callback: CALLBACK_TYPE) -> str:
        ...
        :rtype: str
        """
        return self.subscribe('channel.subscription.gift', '1', {'broadcaster_user_id': broadcaster_user_id}, callback)
    ...
    def listen_channel_subscription_message(self, broadcaster_user_id: str, callback: CALLBACK_TYPE) -> str:
        ...
        :rtype: str
        """
        return self.subscribe('channel.subscription.message',
                               '1',
    ...
    def listen_channel_cheer(self, broadcaster_user_id: str, callback: CALLBACK_TYPE) -> str:
        ...
        :rtype: str
        """
        return self.subscribe('channel.cheer',
                               '1',
                               {'broadcaster_user_id': broadcaster_user_id},
                               callback)
    ...
    def listen_channel_raid(self,
        ...
        :rtype: str
        """
        return self.subscribe('channel.raid',
                               '1',
                               remove_none_values({
    ...
    def listen_channel_ban(self, broadcaster_user_id: str, callback: CALLBACK_TYPE) -> str:
        ...
        :rtype: str
        """
        return self.subscribe('channel.ban',
                               '1',
                               {'broadcaster_user_id': broadcaster_user_id},
                               callback)
    ...
    def listen_channel_unban(self, broadcaster_user_id: str, callback: CALLBACK_TYPE) -> str:
        ...
        :rtype: str
        """
        return self.subscribe('channel.unban',
                               '1',
                               {'broadcaster_user_id': broadcaster_user_id},
                               callback)
    ...
    def listen_channel_moderator_add(self, broadcaster_user_id: str, callback: CALLBACK_TYPE) -> str:
        ...
        :rtype: str
        """
        return self.subscribe('channel.moderator.add',
                               '1',
                               {'broadcaster_user_id': broadcaster_user_id},
                               callback)
    ...
    def listen_channel_moderator_remove(self, broadcaster_user_id: str, callback: CALLBACK_TYPE) -> str:
        ...
        :rtype: str
        """
        return self.subscribe('channel.moderator.remove',
                               '1',
                               {'broadcaster_user_id': broadcaster_user_id},
                               callback)
    ...
    def listen_channel_points_custom_reward_add(self, broadcaster_user_id: str,
        ...
        :rtype: str
        """
        return self.subscribe('channel.channel_points_custom_reward.add',
                               '1',
                               {'broadcaster_user_id': broadcaster_user_id},
                               callback)
    ...
    def listen_channel_points_custom_reward_update(self,
        ...
        :rtype: str
        """
        return self.subscribe('channel.channel_points_custom_reward.update',
                               '1',
                               remove_none_values({
                                   'broadcaster_user_id': broadcaster_user_id,
                                   'reward_id': reward_id}),
    ...
    def listen_channel_points_custom_reward_remove(self,
        ...
        :rtype: str
        """
        return self.subscribe('channel.channel_points_custom_reward.remove',
                               '1',
                               remove_none_values({
                                   'broadcaster_user_id': broadcaster_user_id,
                                   'reward_id': reward_id}),
    ...
    def listen_channel_points_custom_reward_redemption_add(self,
        ...
        :rtype: str
        """
        return self.subscribe('channel.channel_points_custom_reward_redemption.add',
                               '1',
                               remove_none_values({
                                   'broadcaster_user_id': broadcaster_user_id,
                                   'reward_id': reward_id}),
    ...
    def listen_channel_points_custom_reward_redemption_update(self,
        ...
        :rtype: str
        """
        return self.subscribe('channel.channel_points_custom_reward_redemption.update',
                               '1',
                               remove_none_values({
                                   'broadcaster_user_id': broadcaster_user_id,
                                   'reward_id': reward_id}),
    ...
    def listen_channel_poll_begin(self, broadcaster_user_id: str, callback: CALLBACK_TYPE) -> str:
        ...
        :rtype: str
        """
        return self.subscribe('channel.poll.begin',
                               '1',
                               {'broadcaster_user_id': broadcaster_user_id},
                               callback)
    ...
    def listen_channel_poll_progress(self, broadcaster_user_id: str, callback: CALLBACK_TYPE) -> str:
        ...
        :rtype: str
        """
        return self.subscribe('channel.poll.progress',
                               '1',
                               {'broadcaster_user_id': broadcaster_user_id},
                               callback)
    ...
    def listen_channel_poll_end(self, broadcaster_user_id: str, callback: CALLBACK_TYPE) -> str:
        ...
        :rtype: str
        """
        return self.subscribe('channel.poll.end',
                               '1',
                               {'broadcaster_user_id': broadcaster_user_id},
                               callback)
    ...
    def listen_channel_prediction_begin(self, broadcaster_user_id: str, callback: CALLBACK_TYPE) -> str:
        ...
        :rtype: str
        """
        return self.subscribe('channel.prediction.begin',
                               '1',
                               {'broadcaster_user_id': broadcaster_user_id},
                               callback)
    ...
    def listen_channel_prediction_progress(self, broadcaster_user_id: str, callback: CALLBACK_TYPE) -> str:
        ...
        :rtype: str
        """
        return self.subscribe('channel.prediction.progress',
                               '1',
                               {'broadcaster_user_id': broadcaster_user_id},
                               callback)
    ...
    def listen_channel_prediction_lock(self, broadcaster_user_id: str, callback: CALLBACK_TYPE) -> str:
        ...
        :rtype: str
        """
        return self.subscribe('channel.prediction.lock',
                               '1',
                               {'broadcaster_user_id': broadcaster_user_id},
                               callback)
    ...
    def listen_channel_prediction_end(self, broadcaster_user_id: str, callback: CALLBACK_TYPE) -> str:
        ...
        :rtype: str
        """
        return self.subscribe('channel.prediction.end',
                               '1',
                               {'broadcaster_user_id': broadcaster_user_id},
                               callback)
    ...
    def listen_drop_entitlement_grant(self,
        ...
        :rtype: str
        """
        return self.subscribe('drop.entitlement.grant',
                               '1',
                               remove_none_values({
    ...
    def listen_extension_bits_transaction_create(self,
        ...
        :rtype: str
        """
        return self.subscribe('extension.bits_transaction.create',
                               '1',
    ...
    def listen_hype_train_begin(self, broadcaster_user_id: str, callback: CALLBACK_TYPE) -> str:
        ...
        :rtype: str
        """
        return self.subscribe('channel.hype_train.begin',
                               '1',
                               {'broadcaster_user_id': broadcaster_user_id},
                               callback)
    ...
    def listen_hype_train_progress(self, broadcaster_user_id: str, callback: CALLBACK_TYPE) -> str:
        ...
        :rtype: str
        """
        return self.subscribe('channel.hype_train.progress',
                               '1',
                               {'broadcaster_user_id': broadcaster_user_id},
                               callback)
    ...
    def listen_hype_train_end(self, broadcaster_user_id: str, callback: CALLBACK_TYPE) -> str:
        ...
        :rtype: str
        """
        return self.subscribe('channel.hype_train.end',
                               '1',
                               {'broadcaster_user_id': broadcaster_user_id},
                               callback)
    ...
    def listen_stream_online(self, broadcaster_user_id: str, callback: CALLBACK_TYPE) -> str:
        ...
        :rtype: str
        """
        return self.subscribe('stream.online',
                               '1',
                               {'broadcaster_user_id': broadcaster_user_id},
                               callback)
    ...
    def listen_stream_offline(self, broadcaster_user_id: str, callback: CALLBACK_TYPE) -> str:
        ...
        :rtype: str
        """
        return self.subscribe('stream.offline',
                               '1',
                               {'broadcaster_user_id': broadcaster_user_id},
                               callback)
    ...
    def listen_user_authorization_grant(self, client_id: str, callback: CALLBACK_TYPE) -> str:
        ...
        :rtype: str
        """
        return self.subscribe('user.authorization.grant',
                               '1',
                               {'client_id': client_id},
                               callback)
    ...
    def listen_user_authorization_revoke(self, client_id: str, callback: CALLBACK_TYPE) -> str:
        ...
        :rtype: str
        """
        return self.subscribe('user.authorization.revoke',
                               '1',
                               {'client_id': client_id},
                               callback)
    ...
    def listen_user_update(self, user_id: str, callback: CALLBACK_TYPE) -> str:
        ...
        :rtype: str
        """
        return self.subscribe('user.update',
                               '1',
                               {'user_id': user_id},
                               callback)
