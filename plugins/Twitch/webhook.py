...
from .helpers import build_url, TWITCH_API_BASE_URL, get_uuid, get_json, make_fields_datetime, fields_to_enum,\
    extract_uuid_str_from_url
from .types import *
import requests
...
class TwitchWebHook:
    ...
    def renew_subscription(self,
        ...
        return self._subscribe(url.get('callback_path'), url.get('webhook_url'))
    ...
    def unsubscribe(self,
        ...
        if url is None:
            raise Exception(f'no subscription found for UUID {str(uuid)}')
        success = self._generic_unsubscribe(url.get('callback_path'), url.get('webhook_url'))
        if success:
