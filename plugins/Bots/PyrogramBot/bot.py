...
class PyrogramBot:
    ...
    def __init__(self, api_hash: str, api_id: str, bot_token=Optional[str], user_session=':memory:',
        ...
        self.user = Client(session_name=user_session, api_id=api_id, api_hash=api_hash)
        self.bot = Client(session_name=bot_session, bot_token=bot_token, api_id=api_id, api_hash=api_hash)
    ...
    async def set_default_commands(self, bot_commands_default: SetBotCommands = None,
        ...
        if not bot_commands_default:
            bot_commands_default = SetBotCommands(scope=BotCommandScopeDefault(), lang_code='en', commands=[
        ...
        if not bot_commands_users:
            bot_commands_users = SetBotCommands(scope=BotCommandScopeUsers(), lang_code='en', commands=[
                BotCommand(command='start', description="Start command"),
                BotCommand(command='help', description="Help command"),
        ...
        if not bot_commands_chats:
            bot_commands_chats = SetBotCommands(scope=BotCommandScopeChats(), lang_code='en', commands=[
                BotCommand(command='help', description="Help command"),
        ...
        await self.bot.send(bot_commands_default)
        await self.bot.send(bot_commands_users)
        await self.bot.send(bot_commands_chats)

    async def set_default_commands_ru(self):
        bot_commands_default = SetBotCommands(scope=BotCommandScopeDefault(), lang_code='ru', commands=[
            BotCommand(command='start', description="Start команда"),
            BotCommand(command='help', description="Help команда"),
        ])

        bot_commands_users = SetBotCommands(scope=BotCommandScopeUsers(), lang_code='ru', commands=[
            BotCommand(command='start', description="Start команда"),
            BotCommand(command='help', description="Help команда"),
            # BotCommand(command='echo', description="Echo any message (/echo [message])"),
        ...
        bot_commands_chats = SetBotCommands(scope=BotCommandScopeChats(), lang_code='ru', commands=[
            BotCommand(command='help', description="Help команда"),
